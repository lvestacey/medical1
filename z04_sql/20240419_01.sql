...
no number(4,2),
mdate date,
mdate2 timestamp --date: 년,월,일,시,분,초까지 저장가능/timestemp 밀리초까지 저장가능함.
);
...
select * from mem3 where name='aaa'; --sql구문은 대소문자 구분 없음. 데이터는 대소문자 구분함 
...
select * from employees;
-- 사원번호(e_id), 사원이름 (e_name), 급여(salary), 입사일자(hire_date)
...
-- 월급 *12=년봉 
-- 달러년봉, 원화년봉을 구하시오.
select employee_id,emp_name,salary*12*1381.79 from employees;
